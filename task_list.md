...
- [x] 백엔드: `Book`, `BookImage` 엔티티 생성 (ERD 기반)
- [x] 백엔드: `BookController` — 목록 조회, 상세 조회, 등록, 수정, 삭제 API 구현
- [x] 백엔드: 이미지 업로드 처리 (개발 단계: 로컬 파일 저장 / 운영 단계: AWS S3)
- [x] 백엔드: 거래 상태 변경 `PATCH /api/books/{bookId}/status` API 구현
- [x] 프론트엔드: 책 목록(홈) 화면 — 카드형 리스트 UI 및 페이지네이션
- [x] 프론트엔드: 책 상세 조회 화면 — 이미지 슬라이더, 판매자 정보, 상태 표시
- [x] 프론트엔드: 새 책 등록 화면 — 폼 입력 + 다중 이미지 업로드 UI
- [x] 프론트엔드: 책 수정/삭제 기능 연동

### 3-4. 도서 검색 및 필터링 기능
- [x] 백엔드: 검색 쿼리 파라미터 처리 — 제목/강의명/교수명 검색
- [x] 백엔드: 필터링 — 카테고리(전공/교양/자격증), 거래 상태, 학과별 조회
